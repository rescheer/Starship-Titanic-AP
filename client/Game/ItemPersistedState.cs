...
/// distinguish the two (the engine's own `_destRoomFlags` sentinel - see GameActions.MarkItemAsToolPlaced) does
/// not survive to the moment the player actually retrieves the item from the mail system: live testing showed
/// the game's own mail-retrieval processing overwrites that field with a real value of its own before this app
/// ever gets to read it back, so the distinction has to be captured here, in this app's own persisted state, at
/// the moment the item enters Mail - not re-derived later from a volatile engine field.</param>
